...
{#if data?.pages?.length > 0}
  ...
  {#each data.pages as p, i}
    <div class="pr-top-row pr-top-row-clickable" role="button" tabindex="0"
      ...
      onmouseenter={(e) => ontooltip?.({ x: e.clientX, y: e.clientY, url: p.url, pr: p.pagerank, depth: p.depth, intLinks: p.internal_links_out, extLinks: p.external_links_out, words: p.word_count })}
...
<style>
  ...
  .pr-top-rank { text-align: right; font-weight: 700; color: var(--text-muted); font-size: 12px; }
  .pr-top-url { min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; color: var(--accent); cursor: pointer; }
  .pr-top-url:hover { color: var(--accent-hover); }
  .pr-top-bar { height: 22px; border-radius: 4px; background: var(--accent); transition: width 0.3s ease; }
  .pr-top-score { text-align: right; font-weight: 600; color: var(--accent); font-variant-numeric: tabular-nums; }
  .pr-top-badges { display: flex; gap: 4px; }
  .pr-top-badge { font-size: 11px; padding: 2px 6px; border-radius: 4px; background: var(--bg); color: var(--text-muted); font-weight: 500; white-space: nowrap; }
</style>
