<script>
  ...
  let { loading = false, linksResult = null, diffType = 'added', onswitchtype, onpagechange } = $props();
</script>
...
{:else if linksResult && linksResult.links?.length > 0}
  ...
  <Pagination offset={linksResult.offset || 0} limit={100} total={linksResult.links.length < 100 ? (linksResult.offset || 0) + linksResult.links.length : Infinity}
