...
<div class="charts-container">
  {#if stats?.depth_distribution && Object.keys(stats.depth_distribution).length > 0}
    {@const depthEntries = Object.entries(stats.depth_distribution)
      .map(([k, v]) => [Number(k), v])
      .sort((a, b) => a[0] - b[0])}
    ...
    <div class="chart-section">
      ...
      <svg class="chart-svg" viewBox="0 0 600 {depthSvgH}" preserveAspectRatio="xMinYMin meet">
        {#each depthEntries as [depth, count], i}
          ...
          <g
            class="chart-bar-clickable cursor-pointer"
            role="button"
            tabindex="0"
            onclick={() =>
              onnavigate?.(`/sessions/${sessionId}/overview`, { depth: String(depth) })}
            onkeydown={a11yKeydown(() =>
              onnavigate?.(`/sessions/${sessionId}/overview`, { depth: String(depth) }),
            )}
          >
            <text x="30" y={y + depthBarH / 2 + 5} text-anchor="end" class="chart-label"
              >{depth}</text
            >
            <rect
              x="40"
  ...
  {#if stats?.status_codes && Object.keys(stats.status_codes).length > 0}
    {@const scEntries = Object.entries(stats.status_codes)
      .map(([k, v]) => [Number(k), v])
      .sort((a, b) => a[0] - b[0])}
      ...
      <svg class="chart-svg" viewBox="0 0 600 {scSvgH}" preserveAspectRatio="xMinYMin meet">
        {#each scEntries as [code, count], i}
          ...
          <g
            ...
            tabindex="0"
            onclick={() =>
              onnavigate?.(`/sessions/${sessionId}/overview`, { status_code: String(code) })}
            onkeydown={a11yKeydown(() =>
              onnavigate?.(`/sessions/${sessionId}/overview`, { status_code: String(code) }),
            )}
          >
