  ...
  <div class="pr-treemap-container">
    {#each treemapRects as rect}
      ...
      <div class="pr-treemap-rect" role="button" tabindex="0"
        ...
        onmouseenter={(e) => ontooltip?.({ x: e.clientX, y: e.clientY, path: rect.path, pages: rect.page_count, totalPR: rect.total_pr, avgPR: rect.avg_pr, maxPR: rect.max_pr })}
...
<style>
  ...
  .pr-treemap-label { font-size: 11px; font-weight: 600; color: #fff; text-align: center; padding: 4px; line-height: 1.2; text-shadow: 0 1px 2px rgba(0,0,0,0.4); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; max-width: 100%; }
  .pr-treemap-label small { display: block; font-size: 10px; font-weight: 400; opacity: 0.85; }
</style>
