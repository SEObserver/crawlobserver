...
{#if data && data.total_with_pr > 0}
  <div class="stats-grid pr-stats-grid">
    <div class="stat-card">
      <div class="stat-value">{fmtN(data.total_with_pr)}</div>
      <div class="stat-label">{t('pagerank.pagesWithPR')}</div>
    </div>
    <div class="stat-card">
      <div class="stat-value">{data.avg.toFixed(2)}</div>
      <div class="stat-label">{t('pagerank.mean')}</div>
    </div>
    <div class="stat-card">
      <div class="stat-value">{data.median.toFixed(2)}</div>
      <div class="stat-label">{t('pagerank.median')}</div>
    </div>
    <div class="stat-card">
      <div class="stat-value">{data.p90.toFixed(2)}</div>
      <div class="stat-label">P90</div>
    </div>
    <div class="stat-card">
      <div class="stat-value">{data.p99.toFixed(2)}</div>
  ...
  {@const barGap = 1}
  {@const barW =
    distBuckets.length > 0 ? (plotW - (distBuckets.length - 1) * barGap) / distBuckets.length : 0}
  ...
  <svg viewBox="0 0 {histW} {histH}" class="pr-hist-svg">
    {#each [1, 10, 100, 1000, 10000, 100000] as tick}
      {#if tick <= distMaxCount * 1.5}
        {@const ty =
          histMargin.top + plotH - (logMax > 0 ? (Math.log10(tick + 1) / logMax) * plotH : 0)}
        <line
          x1={histMargin.left}
          y1={ty}
          x2={histW - histMargin.right}
          y2={ty}
          stroke="var(--border)"
          stroke-dasharray="3,3"
        />
        <text x={histMargin.left - 8} y={ty + 4} text-anchor="end" class="pr-axis-tick"
    ...
    {#each distBuckets as bucket, i}
      ...
      {#if distBuckets.length <= 25 || i % Math.ceil(distBuckets.length / 10) === 0}
        <text
          x={bx + barW / 2}
          y={histH - histMargin.bottom + 16}
          text-anchor="middle"
          class="pr-axis-label-sm">{bucket.min.toFixed(0)}</text
        >
      {/if}
    {/each}
    <text x={histW / 2} y={histH - 4} text-anchor="middle" class="pr-axis-label"
      >{t('pagerank.score')}</text
    >
    <text
      x={14}
      y={histH / 2}
      text-anchor="middle"
      transform="rotate(-90, 14, {histH / 2})"
      class="pr-axis-label">{t('pagerank.pagesLog')}</text
    >
  </svg>
{:else}
